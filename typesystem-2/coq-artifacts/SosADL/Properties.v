...
Ltac simpl_in_Z :=
  repeat match goal with
             ...
             | H1: interp_in_Z (SosADL.SosADL.BinaryExpression (Some (SosADL.IntegerValue (Some ?l))) (Some "-") (Some ?v)) = Interpreted ?x,
               ...
               |- _ => assert ((l - r)%Z = x) by (unfold interp_in_Z in *; simpl in H1; rewrite H2 in H1; inversion H1; auto);
                     subst; clear H1
             ...
             | H1: interp_in_Z (SosADL.SosADL.BinaryExpression (Some ?v) (Some "+") (Some (SosADL.IntegerValue (Some ?l)))) = Interpreted ?x,
                   H2: interp_in_Z ?v = Interpreted ?r
               |- _ => assert ((r + l)%Z = x) by (unfold interp_in_Z in *; simpl in H1; rewrite H2 in H1; inversion H1; auto);
                     subst; clear H1
             | H1: interp_in_Z (SosADL.SosADL.BinaryExpression (Some ?v) (Some "-") (Some (SosADL.IntegerValue (Some ?l)))) = Interpreted ?x,
                   H2: interp_in_Z ?v = Interpreted ?r
               |- _ => assert ((r - l)%Z = x) by (unfold interp_in_Z in *; simpl in H1; rewrite H2 in H1; inversion H1; auto);
             ...
             | H1: interp_in_Z (SosADL.SosADL.BinaryExpression (Some ?v1) (Some "-") (Some ?v2)) = Interpreted ?x,
               ...
               |- _ => assert ((r1 - r2)%Z = x) by (unfold interp_in_Z in *; simpl in H1; rewrite H2 in H1; rewrite H3 in H1; inversion H1; auto);
                     subst; clear H1
             | H1: interp_in_Z (SosADL.SosADL.BinaryExpression (Some ?v1) (Some "-") (Some (SosADL.SosADL.BinaryExpression (Some ?v2) (Some "mod") (Some ?v3)))) = Interpreted ?x,
               ...
               |- _ => assert ((r1 - (Z.rem r2 r3))%Z = x) by (unfold interp_in_Z in *; simpl in H1; try rewrite H2 in H1; try rewrite H3 in H1; try rewrite H4 in H1; inversion H1; auto);
                     subst; clear H1
             | H1: interp_in_Z (SosADL.SosADL.BinaryExpression (Some ?v1) (Some "-") (Some (SosADL.SosADL.BinaryExpression (Some ?v1) (Some "mod") (Some ?v3)))) = Interpreted ?x,
                   H2: interp_in_Z ?v1 = Interpreted ?r1,
                       H4: interp_in_Z ?v3 = Interpreted ?r3
               |- _ => assert ((r1 - (Z.rem r1 r3))%Z = x) by (unfold interp_in_Z in *; simpl in H1; rewrite H2 in H1; rewrite H4 in H1; inversion H1; auto);
                     subst; clear H1
                   ...
                   H2: interp_in_Z ?v1 = Interpreted ?r1,
                       H4: interp_in_Z ?v3 = Interpreted ?r3
               |- _ => assert (((r1 + 1) - (Z.rem r1 r3))%Z = x) by (unfold interp_in_Z in *; simpl in H1; rewrite H2 in H1; rewrite H4 in H1; inversion H1; auto);
                     subst; clear H1
                      | H1: interp_in_Z (SosADL.SosADL.UnaryExpression (Some "-") (Some ?v)) = Interpreted ?x,
                            H2: interp_in_Z ?v = Interpreted ?r
                        |- _ => assert ((- r)%Z = x) by (unfold interp_in_Z in *; simpl in H1; rewrite H2 in H1; inversion H1; auto);
                              subst; clear H1
         end.
...
Proof.
  ...
  induction l.
  - discriminate.
  - destruct a. destruct name.
    + simpl. intros. destruct (string_dec n e).
      * subst. apply First_Field.
      * apply Next_Field. auto.
    + intros. apply Next_Field. auto. 
Qed.

Lemma distinct_tl: forall {A: Type}  f (hd: A) tl,
...
Lemma distinct_hd: forall {A: Type} f (hd: A) tl,
    (values (f x) for x of (hd :: tl) are distinct according to option_string_dec)
...
Lemma field_type_other: forall h r n t, field_type r n = Some t -> SosADL.Accessors.FieldDecl_name h <> (Some n) -> field_type (h :: r) n = Some t.
Proof.
  intros. destruct h. destruct name.
  - simpl in H0. simpl. case (string_dec n e).
    + intro. elimtype False. subst. apply H0. reflexivity.
    + auto.
  - simpl. auto.
Qed.

Lemma field_type_some: forall r n t, field_type r n = Some t -> exists f, In f r /\ SosADL.Accessors.FieldDecl_name f = Some n.
Proof.
  intros r n t. induction r.
  - discriminate.
  - simpl. case_eq a. destruct name. intro.
    + destruct (string_dec n e).
      * exists a. { split.
               - left. auto.
               - subst. reflexivity. }
...
Lemma field_type_complete:
  forall l n t
    (ND: values (SosADL.Accessors.FieldDecl_name x) for x of l are distinct according to option_string_dec)
  ,
...
Proof.
  ...
  - assert (field_type r n = Some t)
      by ( apply IHfield_has_type; eapply distinct_tl; apply ND ).

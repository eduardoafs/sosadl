...
public abstract class TypeCheckerExpression extends TypeCheckerDataType {
	...
	protected abstract class AbstractUnaryTypeInfo<P extends Type_expression_node> extends StringBasedUnaryTypeInfo<P> {
		...
		@Override
		public Type_expression_node prove(Environment gamma, UnaryExpression e, Type_expression pOperand,
				DataType tOperand) {
	...
	private <T> T range_modulo_min(Expression lmin, Expression lmax, Expression rmin, Expression rmax, Supplier<T> pos,
			Supplier<T> zero, Supplier<T> neg, Supplier<T> divByZero) {
	...
	private <T> T range_modulo_max(Expression lmin, Expression lmax, Expression rmin, Expression rmax, Supplier<T> pos,
			Supplier<T> zero, Supplier<T> neg, Supplier<T> divByZero) {
		if (isLe(createIntegerValue(1), rmin)) {
	...
	private final UnaryTypeInfo2<?> unop2Not = new BooleanUnaryTypeInfo<>("not", this::createType_expression_Not);
	private final UnaryTypeInfo2<?>[] unaryTypeInformations2 = new UnaryTypeInfo2[] { unop2Same, unop2Opposite,
			unop2Not };

	protected abstract class AbstractBinaryTypeInfo<P extends Type_expression_node>
			extends StringBasedBinaryTypeInfo<P> {
		private final DecaFunction<Environment, Expression, DataType, Type_expression, Subtype, Expression, DataType, Type_expression, Subtype, DataType, P> prover;
		...
		@Override
		public Type_expression_node prove(Environment gamma, BinaryExpression e, Type_expression pLeft, DataType tLeft,
				Type_expression pRight, DataType tRight, DataType r) {
			return p(Type_expression_node.class, gamma,
	...
	private final BinaryTypeInfo2<?> binop2Add = new SynthetizingBinaryTypeInfo<>("+",
			(g, le, lt, lp, ls, re, rt, rp, rs, r) -> createType_expression_Add(g, le, lt, ((RangeType) lt).getVmin(),
					((RangeType) lt).getVmax(), re, rt, ((RangeType) rt).getVmin(), ((RangeType) rt).getVmax(), lp, ls,
					rp, rs),
			this::binopSolverAdd);
	...
	private final BinaryTypeInfo2<?> binop2Sub = new SynthetizingBinaryTypeInfo<>("-",
			(g, le, lt, lp, ls, re, rt, rp, rs, r) -> createType_expression_Sub(g, le, lt, ((RangeType) lt).getVmin(),
					((RangeType) lt).getVmax(), re, rt, ((RangeType) rt).getVmin(), ((RangeType) rt).getVmax(), lp, ls,
					rp, rs),
			this::binopSolverSub);
	...
	private final BinaryTypeInfo2<?> binop2Mul = new SynthetizingBinaryTypeInfo<>("*", this::binopProverMul,
			this::binopSolverMul);
	...
	private final BinaryTypeInfo2<?> binop2Div = new SynthetizingBinaryTypeInfo<>("/", this::binopProverDiv,
			this::binopSolverDiv);
	...
	private Type_expression_node binopProverDiv(Environment g, Expression le, DataType dlt, Type_expression lp,
		...
		if (isLe(createIntegerValue(1), rt.getVmin())) {
			...
			return createType_expression_Div_neg(g, le, lt, lt.getVmin(), lt.getVmax(), re, rt, rt.getVmin(),
					rt.getVmax(), r.getVmin(), r.getVmax(), lp, ls, rp, rs,
					expression_le(rt.getVmax(), createOpposite(createIntegerValue(1))), expression_le(r.getVmin(), c1),
					expression_le(r.getVmin(), c2), expression_le(r.getVmin(), c3), expression_le(r.getVmin(), c4),
					expression_le(c1, r.getVmax()), expression_le(c2, r.getVmax()), expression_le(c3, r.getVmax()),
					expression_le(c4, r.getVmax()));
		} else {
	...
	private final BinaryTypeInfo2<?> binop2Mod = new SynthetizingBinaryTypeInfo<>("mod", this::binopProverMod,
			this::binopSolverMod);
	...
	private Type_expression_node binopProverMod(Environment g, Expression le, DataType dlt, Type_expression lp,
		...
		Range_modulo_min min = range_modulo_min(lt.getVmin(), lt.getVmax(), rt.getVmin(), rt.getVmax(),
				() -> createRange_modulo_min_pos(lt.getVmin(), lt.getVmax(), rt.getVmin(), rt.getVmax(), r.getVmin(),
				...
				() -> createRange_modulo_min_zero(lt.getVmin(), lt.getVmax(), rt.getVmin(), rt.getVmax(), r.getVmin(),
						expression_le(createIntegerValue(0), lt.getVmin()),
						expression_le(r.getVmin(), createIntegerValue(0))),
				() -> createRange_modulo_min_neg(lt.getVmin(), lt.getVmax(), rt.getVmin(), rt.getVmax(), r.getVmin(),
	...
	private final BinaryTypeInfo2<?> binop2Equal = new CmpBinaryTypeInfo<>("=", this::createType_expression_Equal,
			this::binopSolverCmp);
	...
	private final BinaryTypeInfo2<?> binop2Diff = new CmpBinaryTypeInfo<>("<>", this::createType_expression_Diff,
			this::binopSolverCmp);
	private final BinaryTypeInfo2<?> binop2Lt = new CmpBinaryTypeInfo<>("<", this::createType_expression_Lt,
			this::binopSolverCmp);
	private final BinaryTypeInfo2<?> binop2Le = new CmpBinaryTypeInfo<>("<=", this::createType_expression_Le,
			this::binopSolverCmp);
	private final BinaryTypeInfo2<?> binop2Gt = new CmpBinaryTypeInfo<>(">", this::createType_expression_Gt,
			this::binopSolverCmp);
	private final BinaryTypeInfo2<?> binop2Ge = new CmpBinaryTypeInfo<>(">=", this::createType_expression_Ge,
			this::binopSolverCmp);
	private final BinaryTypeInfo2<?>[] binaryTypeInformations2 = new BinaryTypeInfo2[] { binop2Add, binop2Sub,
	...
	private Pair<Type_expression_node, DataType> type_expression_node_Field(Environment gamma,
			org.archware.sosadl.sosADL.Field f) {
		if (f.getField() != null && f.getObject() != null) {
			...
			if (p1.getA() != null && p1.getB() != null) {
				...
				return new Pair<>(
						saveProof(f, p(Type_expression_node.class, gamma,
								(gamma_) -> p(Type_expression_node.class, v, (v_) -> p(Type_expression_node.class,
										p1.getB(), (tau_) -> createType_expression_Field(gamma_, f.getObject(),
	...
	private Pair<Type_expression_node, DataType> type_expression_node_Sequence(Environment gamma, Sequence s) {
		if (s.getElements() == null) {
			...
			List<Pair<Expression, Pair<Type_expression, DataType>>> elts = s.getElements().stream()
					.map((x) -> new Pair<>(x, type_expression(gamma, x))).collect(Collectors.toList());
			if (elts.stream().allMatch((x) -> x.getB() != null && x.getB().getA() != null && x.getB().getB() != null)) {
				...
				return new Pair<>(saveProof(s,
						p(Type_expression_node.class, gamma, (gamma_) -> p(Type_expression_node.class, t, (t_) -> {
							Forall<Expression, Ex<DataType, And<Type_expression, Subtype>>> p1 = proveForall(elts,
									Pair::getA, (p) -> {
	...
	private Pair<Type_expression_node, DataType> type_expression_node_Tuple(Environment gamma, Tuple t) {
		if (t.getElements() == null) {
			...
			if (noDuplicate(t.getElements().stream().map((f) -> f.getLabel()))) {
				Collection<Pair<TupleElement, Pair<Type_expression, DataType>>> elts = t.getElements().stream()
						.map((f) -> new Pair<>(f, type_expression(gamma, f.getValue())))
						.collect(Collectors.toCollection(ConcurrentLinkedQueue::new));
				if (elts.stream().allMatch((p) -> p.getB().getA() != null && p.getB().getB() != null)) {
					List<Pair<TupleElement, Pair<Type_expression, FieldDecl>>> elts2 = elts.stream()
					...
					return new Pair<>(saveProof(t, p(Type_expression_node.class, gamma,
							(gamma_) -> p(Type_expression_node.class, tt, (tt_) -> {
								Forall2<TupleElement, FieldDecl, Ex<Expression, And<Equality, Ex<DataType, And<Equality, Type_expression>>>>> p3 = proveForall2(
	...
	private class MethodCallTyper implements Typer {
		...
		public MethodCallTyper(Environment gamma, MethodCall mc, String methodName, DataType selfType,
				Type_expression selfP, List<Pair<Expression, Pair<Type_expression, DataType>>> params) {
			this.gamma = gamma;
			this.mc = mc;
		...
		private Optional<DataType> lookupAndReturnType() {
			...
			List<DataType> sParamsTypes = params.stream().map(Pair::getB).map(Pair::getB)
					.map(TypeCheckerExpression.this::getSubstitute).collect(Collectors.toList());
	...
	private Pair<Type_expression_node, DataType> type_expression_node_MethodCall(Environment gamma, MethodCall mc) {
		...
		if (methodName != null) {
			...
			List<Pair<Expression, Pair<Type_expression, DataType>>> params = ListExtensions.map(mc.getParameters(),
					(p) -> new Pair<>(p, type_expression(gamma, p)));
	...
	private Pair<Type_expression_node, DataType> type_expression_node_IdentExpression(Environment gamma,
			IdentExpression e) {
		if (e.getIdent() == null) {
			...
			if (ec == null) {
				...
				return new Pair<>(
						saveProof(e, p(Type_expression_node.class, gamma, (gamma_) -> p(Type_expression_node.class, t,
	...
	private Pair<Type_expression_node, DataType> type_expression_node_UnaryExpression(Environment gamma,
			UnaryExpression e) {
		if (e.getRight() == null) {
					...
					if (i.isCandidate(e.getOp(), t1)) {
						...
						if (or.isPresent()) {
							...
							return new Pair<>(saveProof(e,
									p(Type_expression_node.class, gamma, (gamma_) -> p(Type_expression_node.class, t1,
	...
	private Pair<Type_expression_node, DataType> type_expression_node_IntegerValue(Environment gamma, IntegerValue e) {
		...
		return new Pair<>(
				saveProof(e, p(Type_expression_node.class, gamma, (gamma_) -> createType_expression_IntegerValue(gamma_,
	...
	class SynthetizingUnaryTypeInfo<P extends Type_expression_node> extends AbstractUnaryTypeInfo<P> {
		...
		public Optional<DataType> immediateType(UnaryExpression e, DataType operand) {
			if (TypeInferenceSolver.containsVariable(operand)) {
				TypeVariable v = inference.createFreshTypeVariable((lb, ub) -> solver.apply(e, getSubstitute(operand)),
						e, null);
	...
	class CmpBinaryTypeInfo<P extends Type_expression_node> extends SynthetizingBinaryTypeInfo<Type_expression_node> {
		public CmpBinaryTypeInfo(String operator,
				TreDecaFunction<Environment, Expression, DataType, Expression, Expression, Expression, DataType, Expression, Expression, Type_expression, Subtype, Type_expression, Subtype, Type_expression_node> constructor,
				TriFunction<BinaryExpression, DataType, DataType, Optional<DataType>> solver) {
			super(operator,
					(g, l, lt, lp, ls, r, rt, rp, rs, dr) -> constructor.apply(g, l, lt, ((RangeType) lt).getVmin(),
							((RangeType) lt).getVmax(), r, rt, ((RangeType) rt).getVmin(), ((RangeType) rt).getVmax(),
							lp, ls, rp, rs),
					solver);
		}
	}
